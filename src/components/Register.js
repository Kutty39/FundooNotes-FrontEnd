...
import {Button, ButtonToolbar, Col, Container, Form, FormControl, FormGroup, Row} from "react-bootstrap";
...
function Register(props) {
    ...
    const regexValid=(e)=>{
        ...
        if(!regex.test(e.target.value)){
            setPasError({...pasError,er: true,msg: "Password is combination of Upper case,Lower case,Number and Special Character it should be min length of 8 and Max 15"});
        }else{
    ...
    const emailvalidator = (e) => {
        ...
        if (emailField.value !== "") {
            axios.get(`/email/${emailField.value}`)
                .then((response) => {
                    response.data.response ? setEmailError("Email already registered") : setEmailError("");
        ...
        <Container className="d-flex justify-content-center my-5">
            <Form noValidate validated={validated} className={"p-5 shadow-lg rounded"} onSubmit={register}>
                ...
                <FormGroup as={Row}>
                        ...
                        <Col>
                            <FormControl name="pas" value={regData.pas} type="password" isInvalid={pasError.er}
                                         required={true} onChange={changeValues} placeholder={"Password"} onBlur={passwordValidator}/>
                ...
                <ButtonToolbar className="py-2">
                    ...
                    <Button size={"md"} className="mr-2" onClick={gotoLogin}>Login</Button>
                </ButtonToolbar>
            </Form>
        </Container>
    )
